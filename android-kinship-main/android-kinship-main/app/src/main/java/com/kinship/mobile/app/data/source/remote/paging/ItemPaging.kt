 ...
 *
 * Creates a [itemPaging] as an extension for [LazyListScope] that create [item] for different
 * loading status.
 *
...
fun <T : Any> LazyListScope.itemPaging(
    ...
    loadingContent: @Composable (() -> Unit)? = { CustomLoader() },
    ...
    when (pagingData.loadState.append) {
        is LoadState.Loading -> loadingContent?.let { item { CustomLoader() } }
